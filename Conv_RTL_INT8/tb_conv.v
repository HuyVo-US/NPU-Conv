...
module tb_conv;
    ...
    reg signed [31:0] output_mem [0:1023];
    ...
    wire signed [7:0] conv_src1_readdata;
    wire signed [7:0] conv_src2_readdata;
    wire signed [7:0] conv_src1_writedata;
    wire conv_src1_write_en;
    wire [13:0] conv_src2_address;
    wire signed [7:0] conv_src2_writedata;
    wire conv_src2_write_en;
    reg signed [31:0] conv_dest_readdata;
    wire [13:0] conv_dest_address;
    wire signed [31:0] conv_dest_writedata;
    ...
    function signed [7:0] int_to_int8;
        input integer value;
        begin
            if (value > 127) begin
                int_to_int8 = 8'sd127;
                ...
                int_to_int8 = value[7:0];
            end
        end
    endfunction

    task init_int8_test_data;
        begin
            for (row = 0; row < conv_src1_row_size; row = row + 1) begin
                for (col = 0; col < conv_src1_col_size; col = col + 1) begin
                    src1.mem[conv_src1_start_address + row * conv_src1_col_size + col] =
                        int_to_int8(((row * conv_src1_col_size + col) % 16) - 8);
                end
            end
            ...
            for (row = 0; row < conv_src2_row_size; row = row + 1) begin
                for (col = 0; col < conv_src2_col_size; col = col + 1) begin
                    src2.mem[conv_src2_start_address + row * conv_src2_col_size + col] =
                        int_to_int8((row * conv_src2_col_size + col) - 4);
                end
            end
        end
    endtask
            ...
            for (row = 0; row < conv_src2_row_size; row = row + 1) begin
                for (col = 0; col < conv_src2_col_size; col = col + 1) begin
                    if (col == conv_src2_col_size - 1) begin
                        $fdisplay(kernel_file, "%02h",
                            src2.mem[conv_src2_start_address + row * conv_src2_col_size + col]);
                    end
                    else begin
                        $fwrite(kernel_file, "%02h ",
                            src2.mem[conv_src2_start_address + row * conv_src2_col_size + col]);
                    end
                end
            end
            ...
            for (row = 0; row < output_row_size; row = row + 1) begin
                for (col = 0; col < output_col_size; col = col + 1) begin
                    ...
                    if (col == output_col_size - 1) begin
                        $fdisplay(output_file, "%08h", output_mem[output_index]);
                    end
                    else begin
                        $fwrite(output_file, "%08h ", output_mem[output_index]);
                    end
                end
            end
        end
    ...
    initial begin
        ...
        for (output_index = 0; output_index < 1024; output_index = output_index + 1) begin
            output_mem[output_index] = 32'sd0;
        end
